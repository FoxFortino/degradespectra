    ...
    for i in range(N):
        ...
        if makegif:
            ...
            if (i % 5 == 0) or (i in np.arange(N - 5, N)):
                ...
                plt.plot(wvl, flux, c="k", lw=2,
                         label="Original Spectrum")
                plt.plot(wvl, G*r, c="tab:blue", lw=2,
                         label=f"Kernel * {r:.2f}")
                plt.plot(wvl, G_max*r, c="gray", ls=":",
                         label=f"Kernel Max * {r:.2f}", marker="o")
                plt.plot(wvl, flux_conv, c="tab:orange", lw=2,
                         label="Convolution")
                ...
                if i == 200:
                    print(imagedir)
                    file = os.path.join(imagedir, f"conv.pdf")
...
def degrade(
    ...
    if isinstance(SNIDobj, SNIDsn.SNIDsn):
        ...
        new_data = np.zeros(1024, dtype=dtypes)
